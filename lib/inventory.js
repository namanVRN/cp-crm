...
function parseMediaLinks(val) {
  ...
  // If there's only one URL and no label/colon formatting, treat it as a single default link
  if (urls.length === 1 && !str.includes(':')) {
    return [{ label: '', url: urls[0] }];
  }

  // Matches patterns like "Label : URL" or "Label: URL", handling quotes, commas, spaces and separators
  const pattern = /([^:\r\n,|]+)\s*:\s*["']?\s*(https?:\/\/[^"'\s]+)\s*["']?/gi;
  const results = [];
  let match;

  while ((match = pattern.exec(str)) !== null) {
    let label = match[1].replace(/\band\b/gi, '').trim(); // Remove optional "and" conjunctions
    label = label.replace(/^[^a-zA-Z0-9]+|[^a-zA-Z0-9]+$/g, '').trim(); // Clean trailing/leading non-alphanumeric chars
    
    if (label) {
      label = label.charAt(0).toUpperCase() + label.slice(1); // Standard Capitalization
    }
    results.push({ label, url: match[2].trim() });
  }

  // Fallback: if regex mapping fails but links exist, return generic labeled items
...
export async function getInventoryDashboardData(sheets, spreadsheetId) {
  ...
  for (let p = 1; p <= 15; p++) {
    ...
    // Parse brochure data (both raw string and labeled array representation)
    if (rowBrochure > -1) {
    ...
    // Parse video walkarounds (both raw string and labeled array representation)
    if (rowVideo > -1) {
